...
// Action mode (add to cart vs. open modal)
$shop_action  = ! empty( $settings->shop_action ) ? $settings->shop_action : 'add_to_cart';
$modal_html   = isset( $settings->modal_content ) ? $settings->modal_content : '';
$modal_id     = 'avalon-modal-' . ( isset( $module->node ) ? esc_attr( $module->node ) : uniqid() );
$is_modal_btn = ( $shop_action === 'modal' && ! empty( trim( $modal_html ) ) );
...
<div class="<?php echo esc_attr( implode( ' ', $wrapper_classes ) ); ?>">
  ...
  <?php if ( $is_modal_btn ) : ?>
    <button type="button"
            class="avalon-button avalon-button-<?php echo $shop_style; ?> avalon-modal-trigger"
...
<?php
if ( $is_modal_btn ) {
  AvalonButton::register_modal( $modal_id, $modal_html );
}
?>
