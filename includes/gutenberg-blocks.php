...
/**
 * Register Hero Split block
 */
function vestelli_register_hero_split_block() {
	if ( ! function_exists( 'register_block_type' ) ) {
		return;
	}

	$block_path = get_stylesheet_directory() . '/blocks/hero-split';
...
add_action( 'init', 'vestelli_register_hero_split_block', 20 );

/**
 * Enqueue Hero Split block editor assets so the editor.js loads even when
 * block.json's editorScript fails to register (matches the hero block fallback).
 */
function vestelli_enqueue_hero_split_block_editor_assets() {
	$block_path = get_stylesheet_directory() . '/blocks/hero-split';
	$block_url  = get_stylesheet_directory_uri() . '/blocks/hero-split';

	$editor_js = $block_path . '/editor.js';
	if ( file_exists( $editor_js ) ) {
		wp_enqueue_script(
			'vestelli-hero-split-block-editor',
			$block_url . '/editor.js',
			array(
				'wp-blocks',
				'wp-element',
...
add_action( 'enqueue_block_editor_assets', 'vestelli_enqueue_hero_split_block_editor_assets' );

/**
 * Register Suunnittelija-aineistot block
 */
function vestelli_register_aineistot_block() {
	if ( ! function_exists( 'register_block_type' ) ) {
		return;
	}

	$block_path = get_stylesheet_directory() . '/blocks/aineistot';
	...
	register_block_type_from_metadata(
		$block_path,
		array( 'render_callback' => 'vestelli_render_aineistot_block' )
	);
}
add_action( 'init', 'vestelli_register_aineistot_block', 20 );

/**
 * Render callback for Suunnittelija-aineistot block.
 */
function vestelli_render_aineistot_block( $attributes, $content, $block ) {
	$template = get_stylesheet_directory() . '/blocks/aineistot/render.php';
...
/**
 * Enqueue Suunnittelija-aineistot block editor assets.
 *
 * Mirrors the hero block enqueue, plus adds wp-server-side-render so the
 * canvas live-preview component is available.
 */
function avalon_enqueue_aineistot_block_editor_assets() {
	$block_path = get_stylesheet_directory() . '/blocks/aineistot';
	$block_url  = get_stylesheet_directory_uri() . '/blocks/aineistot';

	$editor_js = $block_path . '/editor.js';
	if ( file_exists( $editor_js ) ) {
		wp_enqueue_script(
			'vestelli-aineistot-block-editor',
			$block_url . '/editor.js',
			array(
				'wp-blocks',
				'wp-element',
...
add_action( 'enqueue_block_editor_assets', 'avalon_enqueue_aineistot_block_editor_assets' );
