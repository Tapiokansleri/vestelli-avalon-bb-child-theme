...
(function() {
  ...
  function setupDesktopMegaMenu() {
    ...
    function tagMegaItems(submenu) {
      var inner = submenu && submenu.querySelector(':scope > .kmm-submenu__inner');
      if (!inner) {
        return;
      }
      Array.prototype.forEach.call(inner.children, function(col) {
        if (col.tagName !== 'LI') {
          return;
        }
        col.classList.add('avalon-mega-column');
        var heading = getDirectChild(col, 'a');
        if (heading) {
          heading.classList.add('avalon-mega-heading');
        }
        var nestedSub = getDirectChild(col, '.sub-menu') || getDirectChild(col, 'ul');
        if (nestedSub) {
          nestedSub.querySelectorAll('a').forEach(function(link) {
            link.classList.add('avalon-mega-link');
          });
        }
      });
    }
